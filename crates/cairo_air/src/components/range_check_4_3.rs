...
impl<Value: IValue> CircuitEval<Value> for Component {
    ...
    fn evaluate(
        ...
        // Verify this component has 2 ** 7 rows
        let size_bit = component_data.get_n_instances_bit(context, 7);
...
mod tests {
    ...
    fn test_evaluation_result() {
        ...
        let preprocessed_columns = HashMap::from([
            (
                PreProcessedColumnId { id: "range_check_4_3_column_0".to_owned() },
                context.constant(qm31_from_u32s(998303435, 170695910, 1979882780, 534397045)),
            ),
            (
                PreProcessedColumnId { id: "range_check_4_3_column_1".to_owned() },
                context.constant(qm31_from_u32s(1065412614, 304913638, 2046991644, 534397045)),
            ),
        ]);
        ...
        assert_eq!(result_value, RANGE_CHECK_4_3_SAMPLE_EVAL_RESULT)
    }
}
