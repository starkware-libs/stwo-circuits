...
pub const RELATION_USES_PER_ROW: [RelationUse; 2] = [
    ...
    RelationUse { relation_id: "PedersenAggregatorWindowBits18", uses: 1 },
];
...
pub fn accumulate_constraints<Value: IValue>(
    ...
    let instance_addr_tmp_3bd90_0 =
        eval!(context, ((seq) * (3)) + (pedersen_builtin_segment_start));

    read_id::accumulate_constraints(
        &[eval!(context, instance_addr_tmp_3bd90_0), eval!(context, input_state_0_id_col0)],
        context,
        component_data,
        acc,
    );
    ...
    read_id::accumulate_constraints(
        &[eval!(context, (instance_addr_tmp_3bd90_0) + (1)), eval!(context, input_state_1_id_col1)],
        context,
        component_data,
        acc,
    );
    ...
    read_id::accumulate_constraints(
        &[eval!(context, (instance_addr_tmp_3bd90_0) + (2)), eval!(context, output_state_id_col2)],
        context,
        component_data,
        acc,
    );

    // Use PedersenAggregatorWindowBits18.
    let tuple_4 = &[
...
mod tests {
    ...
    fn test_evaluation_result() {
        ...
        let public_params = HashMap::from([(
            "pedersen_builtin_segment_start".to_owned(),
            context.constant(1353712625.into()),
        )]);
        ...
        assert_eq!(result_value, PEDERSEN_BUILTIN_SAMPLE_EVAL_RESULT)
    }
}
