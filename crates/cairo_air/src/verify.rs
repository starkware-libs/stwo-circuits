...
pub fn build_fixed_cairo_circuit(
    ...
    let components = zip_eq(all_components(), &config.enabled_bits)
        .filter_map(
            |((name, component), enable_bit)| {
                if *enable_bit { Some((name, component)) } else { None }
            },
        )
        .collect();
...
pub fn build_cairo_verifier_circuit(verifier_config: &CairoVerifierConfig) -> Context<NoValue> {
    let config = &verifier_config.proof_config;
    let components = zip_eq(all_components::<NoValue>(), &config.enabled_bits)
        .filter_map(
            |((name, component), enable_bit)| {
                if *enable_bit { Some((name, component)) } else { None }
            },
