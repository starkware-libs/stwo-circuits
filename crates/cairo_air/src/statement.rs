...
impl<Value: IValue> CairoStatement<Value> {
    ...
    pub fn verify_builtins(
        ...
        let instance_size_inverses =
            pack_into_qm31s(instance_sizes.iter().map(|(_name, size)| M31::from(*size).inverse()))
                .into_iter()
        ...
        let max_builtin_instance_size =
            instance_sizes.iter().map(|(_name, size)| size).max().unwrap();
        assert!(
        ...
        for ((name, _size), actual_uses) in zip_eq(instance_sizes, actual_uses_iter) {
