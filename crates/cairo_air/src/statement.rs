...
use stwo_cairo_common::builtins::{
    PEDERSEN_BUILTIN_MEMORY_CELLS, POSEIDON_BUILTIN_MEMORY_CELLS, RANGE_CHECK_BUILTIN_MEMORY_CELLS,
};
...
impl<Value: IValue> CairoStatement<Value> {
    ...
    pub fn verify_builtins(
        ...
        let builtin_memory_cells = [
            ...
            ("bitwise_builtin", RANGE_CHECK_BUILTIN_MEMORY_CELLS),
...
impl<Value: IValue> Statement<Value> for CairoStatement<Value> {
    ...
    fn verify_claim(
        ...
        enable_bits: &[Var],
        component_sizes: &[Var],
        relation_uses: &HashMap<&'static str, Var>,
    ) {
        ...
        let opcode_relation_uses = Simd::from_packed(vec![relation_uses["Opcodes"]], 1);
        extract_bits(
            context,
            &opcode_relation_uses,
