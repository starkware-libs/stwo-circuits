...
pub fn collect_oods_responses(
    ...
    let period_generators = period_generators(context, trace_gen, component_sizes);
    let periodicity_sample_points_per_component =
        period_generators.into_iter().map(|pt| add_points(context, &oods_point, &pt)).collect_vec();
