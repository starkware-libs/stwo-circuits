...
use crate::circuits::ops::{Guess, eq, select};
...
/// `root`.
pub fn merkle_path<Value: IValue>(
    ...
    for (bit, sibling) in zip_eq(bits, &auth_path.0) {
        ...
        let left0 = select(context, *bit, leaf.0, sibling.0);
        let left1 = select(context, *bit, leaf.1, sibling.1);

        let right0 = eval!(context, ((leaf.0) + (sibling.0)) - (left0));
        let right1 = eval!(context, ((leaf.1) + (sibling.1)) - (left1));
...
pub fn decommit_eval_domain_samples<Value: IValue>(
    ...
    for (trace_idx, root) in roots.iter().enumerate() {
        for (query_idx, data) in eval_domain_samples.data_for_trace(trace_idx).iter().enumerate() {
            ...
            merkle_path(context, leaf, &bits_for_query, *root, auth_path);
        }
    }
}
