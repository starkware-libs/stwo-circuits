...
pub fn compute_composition_polynomial<Value: IValue>(
    ...
    let mut preprocessed_columns = HashMap::new();
    for (id, value) in
        zip_eq(statement.get_preprocessed_column_ids(), oods_samples.preprocessed_columns)
    {
        preprocessed_columns.insert(id.clone(), *value);
    }
