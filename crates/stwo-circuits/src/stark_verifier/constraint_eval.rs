...
impl<'a> CompositionConstraintAccumulator<'a> {
    ...
    pub fn set_enable_bit(&mut self, context: &mut Context<impl IValue>, enable_bit: Var) {
        self.enable_bit = enable_bit;

        // Baised enable bit is 0 if enable_bit is one, otherwise -1.
        let baised_enable_bit = eval!(context, (self.enable_bit) - (context.one()));

        // correction is zero if enable_bit is one, otherwise -(coeff - 1).
        let correction = eval!(
            context,
            ((self.composition_polynomial_coeff) + (baised_enable_bit)) * (baised_enable_bit)
        );

        // TODO(ilya): Consider chainging the prover to multiple by coeff even if the component is
        // disabled.
        self.one_or_coeff = eval!(context, (self.composition_polynomial_coeff) + (correction));
    }
