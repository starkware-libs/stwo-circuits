...
/// sum(uses_per_row * ceil(num_rows / DIV)) < floor(P / DIV)
...
fn check_relation_uses<Value: IValue>(
    ...
    // Check that sum(uses_per_row * ceil(num_rows / DIV)) cannot overflow even for the maximal
    // num_rows (num_rows = P).
    ...
    // Compute ceil(num_rows / DIV) for all components
    ...
    let shifted_component_sizes = Simd::add(context, &shifted_component_sizes, &one);
    let shifted_component_sizes = Simd::unpack(context, &shifted_component_sizes);

    // Sum uses_per_row * ceil(num_rows / DIV) for all relations
    let mut shifted_relation_uses = HashMap::new();
    for (shifted_size, component) in shifted_component_sizes.into_iter().zip_eq(components.iter()) {
        let relation_uses = component.relation_uses_per_row();
        if relation_uses.is_empty() {
            context.mark_as_unused(shifted_size);
        } else {
            for relation_use in component.relation_uses_per_row() {
                let entry =
