...
pub fn verify(
    ...
    // Select queries.
    let select_queries_input = get_select_queries_input(context, &mut channel, config.n_queries());
    let _queries =
        select_queries(context, &select_queries_input, config.log_evaluation_domain_size());
