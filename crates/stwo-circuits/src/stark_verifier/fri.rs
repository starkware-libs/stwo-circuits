...
use crate::stark_verifier::merkle::{hash_leaf, merkle_path};
...
pub fn fri_decommit<Value: IValue>(
    ...
    for _ in 0..(proof.commit.layer_commitments.len() - 2) {
    ...
    for (tree_idx, (root, twiddles)) in
        zip_eq(&proof.commit.layer_commitments, all_twiddles).enumerate()
    {
        // Check merkle decommitment.
        for (query_idx, fri_query) in fri_data.iter().enumerate() {
            let leaf = hash_leaf(context, *fri_query);
            let auth_path = proof.auth_paths.at(tree_idx, query_idx);
            let bits_for_query = bits.iter().map(|b| b[query_idx]).collect_vec();
            merkle_path(context, leaf, &bits_for_query[tree_idx..], *root, &auth_path);
        }

        // Compute the next layer.
        fri_data = zip_eq(zip_eq(fri_data, &proof.fri_siblings[tree_idx]), twiddles)
    ...
    let last_layer_val = proof.commit.last_layer_coefs[0];
