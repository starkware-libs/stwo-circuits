...
impl Simd {
    ...
    pub fn eq(context: &mut Context<impl IValue>, a: &Simd, b: &Simd) {
        ...
        // Sanity check: the lengths of the packed arrays must be the same.
        assert_eq!(a.data.len(), b.data.len());
        for i in 0..a.data.len() {
            // The number of elements remaining to compare.
            let rem = a.len - i * 4;
            if rem < 4 {
                let diff = eval!(context, (a.data[i]) - (b.data[i]));
                let mask = first_ones(context, rem);
