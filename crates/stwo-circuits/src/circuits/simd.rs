...
impl Simd {
    ...
    pub fn pow2<const N_BITS: usize>(
        ...
        // multiplication.
        let one = context.one();
        let mut bit_val = eval!(context, (one) + (one));
        for (bit_idx, bit) in bits.iter().enumerate() {
            // repeat the bit_val to make it the same length as res,
            let res_if_bit_is_one =
                Simd::scalar_mul(context, &res, &M31Wrapper::new_unsafe(bit_val));
            res = Simd::select(context, bit, &res, &res_if_bit_is_one);
            if bit_idx < N_BITS - 1 {
                bit_val = eval!(context, (bit_val) * (bit_val));
            }
        }
        res
    }
