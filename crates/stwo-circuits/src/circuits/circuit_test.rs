use crate::circuits::circuit::{Add, Circuit, Eq, Mul, Sub, Var};

#[test]
fn circuit_add() {
    let mut c = Circuit { n_vars: 3, ..Circuit::default() };
    c.add.push(Add { in0: Var { idx: 0 }, in1: Var { idx: 1 }, out: Var { idx: 2 } });
...
fn circuit_sub() {
    ...
    c.sub.push(Sub { in0: Var { idx: 0 }, in1: Var { idx: 1 }, out: Var { idx: 3 } });
...
fn circuit_mul() {
    let mut c = Circuit::default();
    c.mul.push(Mul { in0: Var { idx: 1 }, in1: Var { idx: 1 }, out: Var { idx: 2 } });
...
fn circuit_eq() {
    let mut c = Circuit { n_vars: 3, ..Circuit::default() };
    c.eq.push(Eq { in0: Var { idx: 1 }, in1: Var { idx: 2 } });
