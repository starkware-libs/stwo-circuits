    ...
    pub fn new(context: &mut Context<Value>, outputs: &[(M31, [M31; 4])]) -> Self {
        let outputs = outputs
            ...
            .map(|&(addr, [v0, v1, v2, v3])| Output {
                address: M31Wrapper::new_unsafe(context.constant(addr.into())),
                value: [
                    M31Wrapper::new_unsafe(context.constant(v0.into())),
                    M31Wrapper::new_unsafe(context.constant(v1.into())),
                    M31Wrapper::new_unsafe(context.constant(v2.into())),
                    M31Wrapper::new_unsafe(context.constant(v3.into())),
                ],
            })
            .collect();
