...
pub fn generate_interaction_trace(
    ...
    for vec_row in 0..(1 << (LOG_N_INSTANCES - LOG_N_LANES)) {
        let q0: PackedSecureField = lookup_elements
            .combine(&[trace[2].values.data[vec_row], trace[3].values.data[vec_row]]);
        col_gen.write_frac(vec_row, PackedSecureField::one(), q0);
    }
