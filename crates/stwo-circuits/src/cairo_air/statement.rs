...
impl<Value: IValue> CairoStatement<Value> {
    pub fn validate_claim(&self, context: &mut Context<Value>) {
        let initial_state = &self.public_data.initial_state;
        let final_state = &self.public_data.final_state;
        let initial_pc = initial_state.pc;
        let initial_ap = initial_state.ap;
        let initial_fp = initial_state.fp;

        let final_pc = final_state.pc;
        let final_ap = final_state.ap;
        let final_fp = final_state.fp;
