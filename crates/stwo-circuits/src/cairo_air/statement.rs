...
impl PublicData<Var> {
    ...
    pub fn parse_from_vars(data: &[Var]) -> Self {
        ...
        let program_len = *iter.next().unwrap();
        // TODO(ilya): Verify that program_len is consistent with the PROGRAM_LEN.
        for _ in 0..PROGRAM_LEN {
            program.push(get_public_mem_value(&mut iter));
        }
        assert!(iter.next().is_none(), "Expected end of public data");
        Self {
            initial_state,
            final_state,
            public_memory: PublicMemory { segement_ranges, safe_call_ids, output, program_len, program },
        }
    }
}
