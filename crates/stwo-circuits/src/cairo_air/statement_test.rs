...
fn test_public_logup_sum() {
    ...
    let initial_state = CasmState {
        pc: context.new_var(qm31_from_u32s(1, 0, 0, 0)),
        ap: context.new_var(qm31_from_u32s(1336, 0, 0, 0)),
        fp: context.new_var(qm31_from_u32s(1336, 0, 0, 0)),
    };

    // Create final state: CasmState uses { pc, ap, fp } order
    // Cairo1 has: pc=5, ap=2520, fp=1336
    let final_state = CasmState {
        pc: context.new_var(qm31_from_u32s(5, 0, 0, 0)),
        ap: context.new_var(qm31_from_u32s(2520, 0, 0, 0)),
        fp: context.new_var(qm31_from_u32s(1336, 0, 0, 0)),
    };
    ...
    let segment_ranges = [
        ...
        SegmentRange {
            start: PubMemoryM31Value {
                id: context.new_var(qm31_from_u32s(228, 0, 0, 0)),
                value: context.new_var(qm31_from_u32s(2520, 0, 0, 0)),
            },
        ...
        SegmentRange {
            start: PubMemoryM31Value {
                id: context.new_var(qm31_from_u32s(5, 0, 0, 0)),
                value: context.new_var(qm31_from_u32s(0, 0, 0, 0)),
            },
            end: PubMemoryM31Value {
                id: context.new_var(qm31_from_u32s(5, 0, 0, 0)),
                value: context.new_var(qm31_from_u32s(0, 0, 0, 0)),
    ...
    let safe_call_id_0 = context.new_var(qm31_from_u32s(227, 0, 0, 0));
    let safe_call_id_1 = context.new_var(qm31_from_u32s(5, 0, 0, 0));
