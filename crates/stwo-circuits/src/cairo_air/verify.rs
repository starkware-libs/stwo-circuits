...
fn single_claimed_sum<T>(claims: &[T], get_sum: impl FnOnce(&T) -> SecureField) -> SecureField {
    let [claim] = claims else { panic!("expected exactly one component") };
    get_sum(claim)
}
...
fn get_cairo_claimed_sums(interaction_claim: &CairoInteractionClaim) -> Vec<QM31> {
    ...
    claimed_sums.push(single_claimed_sum(assert_eq_double_deref, |c| c.claimed_sum));
    ...
    claimed_sums
        .push(range_check_128_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
    claimed_sums
        .push(range_check_96_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
    claimed_sums.push(bitwise_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
    claimed_sums.push(add_mod_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
    claimed_sums.push(mul_mod_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
    claimed_sums.push(pedersen_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
    claimed_sums.push(poseidon_builtin.as_ref().map_or(SecureField::zero(), |c| c.claimed_sum));
