...
pub fn accumulate_constraints<Value: IValue>(
    ...
    let bitwise_xor_4_0 =
        acc.get_preprocessed_column(&PreProcessedColumnId { id: "bitwise_xor_4_0".to_owned() });
    let bitwise_xor_4_1 =
        acc.get_preprocessed_column(&PreProcessedColumnId { id: "bitwise_xor_4_1".to_owned() });
    let bitwise_xor_4_2 =
        acc.get_preprocessed_column(&PreProcessedColumnId { id: "bitwise_xor_4_2".to_owned() });

    // Yield VerifyBitwiseXor_4.
    let tuple_0 = &[
        eval!(context, 45448144),
        eval!(context, bitwise_xor_4_0),
        eval!(context, bitwise_xor_4_1),
        eval!(context, bitwise_xor_4_2),
    ];
...
impl<Value: IValue> CircuitEval<Value> for Component {
    fn name(&self) -> String {
        "verify_bitwise_xor_4".to_string()
    }
    ...
    fn evaluate(
        ...
        // Verify this component has 2 ** 8 rows
        let size_bit = component_data.get_n_instances_bit(context, 8);
    ...
    fn test_evaluation_result() {
        ...
        let preprocessed_columns = HashMap::from([
            (
                ...
                context.constant(qm31_from_u32s(1700437256, 1297136628, 1235543940, 1503106128)),
            ),
            (
                PreProcessedColumnId { id: "bitwise_xor_4_1".to_owned() },
                context.constant(qm31_from_u32s(1767546435, 1431354356, 1302652804, 1503106128)),
            ),
            (
                PreProcessedColumnId { id: "bitwise_xor_4_2".to_owned() },
                context.constant(qm31_from_u32s(1566218898, 1028701172, 1101326212, 1503106128)),
            ),
        ]);
        ...
        assert_eq!(result_value, VERIFY_BITWISE_XOR_4_SAMPLE_EVAL_RESULT)
    }
}
