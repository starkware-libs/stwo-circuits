...
/// Adds the binary operation gates to the qm31 ops preprocessed trace, in the following format:
...
/// Adds the eq gates to the eq preprocessed trace, in the following format:
...
fn add_qm31_ops_to_preprocessed_trace(
    ...
    let Circuit { n_vars: _, add, sub, mul, pointwise_mul, eq: _, blake: _ } = circuit;
