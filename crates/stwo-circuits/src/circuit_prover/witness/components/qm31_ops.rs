...
pub fn get_columns_from_context(
    ...
    multizip((in0_address, in1_address, out_address)).for_each(
        |(in0_address, in1_address, out_address)| {
            inputs.push([
                values[*in0_address].to_m31_array(),
                values[*in1_address].to_m31_array(),
                values[*out_address].to_m31_array(),
            ]);
        },
    );
...
pub fn write_trace(
    ...
    let inputs = get_columns_from_context(in0_address, in1_address, out_address, values);
    ...
    let (trace, _lookup_data) = write_trace_simd(packed_inputs, preprocessed_columns);
