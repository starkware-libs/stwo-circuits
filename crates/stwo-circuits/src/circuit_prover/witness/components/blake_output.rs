...
#![allow(unused_parens)]
use cairo_air::components::blake_output::{Claim, InteractionClaim, N_TRACE_COLUMNS};
...
fn write_trace_simd(
    ...
    let final_state_addr = preprocessed_trace.get_column(&PreProcessedColumnId {
        id: "final_state_addr".to_owned(),
    });
    let blake_output0_addr = preprocessed_trace.get_column(&PreProcessedColumnId {
        id: "blake_output0_addr".to_owned(),
    });
    let blake_output1_addr = preprocessed_trace.get_column(&PreProcessedColumnId {
        id: "blake_output1_addr".to_owned(),
    });
    let mults = preprocessed_trace.get_column(&PreProcessedColumnId {
        id: blake_output_mults,
    });
    ...
    (
        ...
        .for_each(|(row_index, (row, lookup_data, blake_output_input))| {
            let final_state_addr = final_state_addr.packed_at(row_index);
            let blake_output0_addr = blake_output0_addr.packed_at(row_index);
            let blake_output1_addr = blake_output1_addr.packed_at(row_index);
