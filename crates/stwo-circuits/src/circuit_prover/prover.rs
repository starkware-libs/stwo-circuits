...
pub fn prove(context: Context<QM31>) {
    ...
    // The trace size is the size of the largest column in the preprocessed trace (since all components have preprocessed columns).
    let trace_log_size = preprocessed_trace.iter().map(|col| col.length()).max().unwrap().ilog2();
