...
pub fn prove_fibonacci(mut _context: Context<QM31>) {
    ...
    // Precompute twiddles.
    // Double the size of the composition polynomial domains because we compute on a half-coset, and account for blowup factor.
    ...
    // Preprocessed trace.
    let preprocessed_trace = PreProcessedTrace::fibonacci_preprocessed().gen_trace();
