...
impl PreprocessedCircuit {
    ...
    pub fn from_finalized_circuit(circuit: &Circuit) -> Self {
        ...
        // Add Blake columns.
        let n_blake_compress =
            add_blake_to_preprocessed_trace(circuit, &multiplicities, &mut pp_trace);
        let log_n_blake_compress = n_blake_compress.ilog2();
        ...
        log_seq_sizes.extend([log_n_blake_compress, 4, 15, 16]);
        log_seq_sizes.sort();
        log_seq_sizes.dedup();
        ...
        let blake_g_log_size = log_n_blake_compress + 7;
