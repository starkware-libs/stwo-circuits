...
/// Returns the log size of blake gate component rows.
fn add_blake_to_preprocessed_trace(
    ...
    let n_blake_compress = fill_blake_columns(blake, multiplicities, &mut blake_columns);
...
impl PreprocessedCircuit {
    ...
    pub fn from_finalized_circuit(circuit: &Circuit) -> Self {
        ...
        log_seq_sizes.extend([log_n_blake_gate_rows, 4, 15, 16]);
        log_seq_sizes.sort();
        log_seq_sizes.dedup();
        ...
        // 2. BlakeG trace size (= number of blake compress * 2^7).
        let max_pp_trace_log_size = pp_trace.log_sizes().into_iter().max().unwrap();
        let blake_g_log_size = log_n_blake_gate_rows + 7;
