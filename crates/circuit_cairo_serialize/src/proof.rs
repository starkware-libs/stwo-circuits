...
pub fn prepare_circuit_proof_for_cairo_verifier<H: MerkleHasherLifted<Hash = Blake2sHash>>(
    ...
    CairoSerialize::serialize(&CairoCircuitProof::<H>::from(circuit_proof), &mut felts);
    felts
}

impl<H: MerkleHasherLifted<Hash = Blake2sHash>> From<CircuitProof<H>> for CairoCircuitProof<H> {
    fn from(proof: CircuitProof<H>) -> Self {
        let stark_proof = proof.stark_proof.proof;

        let [trace_log_sizes, interaction_log_sizes] = proof.claim.column_log_sizes_per_tree();
        ...
        Self {
            claim: CairoCircuitClaim::from(&proof.claim),
            interaction_pow: proof.interaction_pow_nonce,
            interaction_claim: CairoCircuitInteractionClaim::from(&proof.interaction_claim),
            stark_proof,
