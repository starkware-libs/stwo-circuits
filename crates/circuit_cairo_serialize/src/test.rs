...
use circuit_prover::prover::{
    BaseColumnPool, SimdBackend, prove_circuit_assignment,
};
...
use crate::proof::CairoCircuitProof;
...
fn test_serialize_deserialize_cairo_proof() {
    ...
    let prover_output = prove_circuit_assignment(
        ctx.values(),
        &preprocessed_circuit,
        &BaseColumnPool::<SimdBackend>::new(),
        PcsConfig::default(),
    );
    assert!(prover_output.stark_proof.is_ok(), "prover failed");

    // Build the Cairo-form proof.
    let proof = CairoCircuitProof::from_prover_output(&prover_output).unwrap();
    ...
    let deserialized: CairoCircuitProof = CairoCircuitProof::deserialize(&mut iter);
    assert!(iter.next().is_none(), "trailing data after proof");
    let mut bytes_after = Vec::new();
    CairoSerialize::serialize(&deserialized, &mut bytes_after);
    assert_eq!(bytes, bytes_after);
}

#[test]
fn test_serialize_deserialize_config() {
    let config = CairoCircuitVerifierConfig {
        output_addresses: vec![10, 20, 30],
        n_blake_gates: 7,
        preprocessed_root: Blake2sHash([0xAB; 32]),
        preprocessed_column_log_sizes: vec![4, 15, 16, 18],
        lifting_log_size: 23,
    };
    ...
    let mut iter = bytes.iter();
    let deserialized = CairoCircuitVerifierConfig::deserialize(&mut iter);
    assert!(iter.next().is_none(), "trailing data after config");
...
fn test_serialize_deserialize_claim_and_interaction_claim() {
    ...
    // Roundtrip claim
    let mut bytes = Vec::new();
    CairoSerialize::serialize(&claim, &mut bytes);
    let mut iter = bytes.iter();
    ...
    let mut bytes = Vec::new();
    CairoSerialize::serialize(&interaction, &mut bytes);
    let mut iter = bytes.iter();
