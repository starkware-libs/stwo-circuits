...
/// serializes via Cairo `Serde` as the bare concatenation of its elements (no length prefix), so
/// the Cairo side must mirror this as a fixed-size sequence of claimed sums in the same sorted
/// order.
#[derive(Clone, Debug, PartialEq, Eq, CairoSerialize, CairoDeserialize)]
