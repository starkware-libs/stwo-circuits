...
impl<Value: IValue> Statement<Value> for CircuitStatement<Value> {
    ...
    fn public_logup_sum(
        ...
        // Output gates public logup contribution.
        ...
        // Blake IV public logup contribution (used by blake gates but never yielded).
        if self.n_blake_gates > 0 {
            let initial_state = crate::blake2s_initial_state();
            let state_id = eval!(context, 1061955672);
            let padding = context.zero();
            let mut blake_iv_elements = vec![state_id, padding];
            for &word in initial_state.iter() {
                let low = context.constant(QM31::from(M31::from(word & 0xffff)));
                let high = context.constant(QM31::from(M31::from((word >> 16) & 0xffff)));
                blake_iv_elements.push(low);
                blake_iv_elements.push(high);
            }
            let blake_iv_term = logup_use_term(context, &blake_iv_elements, interaction_elements);
            let n_blake = context.constant(QM31::from(M31::from(self.n_blake_gates as u32)));
            let blake_iv_yield = eval!(context, (n_blake) * (blake_iv_term));
