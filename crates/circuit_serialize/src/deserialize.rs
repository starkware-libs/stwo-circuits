...
impl CircuitDeserialize for M31 {
    fn deserialize(data: &mut &[u8]) -> DeserializeResult<Self> {
        ...
        // Fold the top bit into the lower 31 bits: since P = 2^31-1, the top bit wraps into +1,
        // giving a value in [0, P] that one partial_reduce brings to [0, P-1].
        let val = u32::from_le_bytes(bytes);
        Ok(M31::partial_reduce((val >> 31) + (val & P)))
    }
}
...
fn take_bytes<'a>(data: &mut &'a [u8], n: usize) -> DeserializeResult<&'a [u8]> {
    data.split_off(..n).ok_or(DeserializeError)
}
