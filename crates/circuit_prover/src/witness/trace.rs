...
pub fn write_interaction_trace(
    ...
    let common_lookup_elements = &interaction_elements.common_lookup_elements;

    // Run all component interaction traces in parallel.
    let (
        (eq_trace, eq_claimed_sum),
        (qm31_ops_trace, qm31_ops_claimed_sum),
        (blake_gate_trace, blake_gate_interaction_claim),
        (blake_round_trace, blake_round_interaction_claim),
        (blake_round_sigma_trace, blake_round_sigma_interaction_claim),
        (blake_g_trace, blake_g_interaction_claim),
        (blake_output_trace, blake_output_interaction_claim),
        (triple_xor_32_trace, triple_xor_32_interaction_claim),
        (verify_bitwise_xor_8_trace, verify_bitwise_xor_8_interaction_claim),
        (verify_bitwise_xor_12_trace, verify_bitwise_xor_12_interaction_claim),
        (verify_bitwise_xor_4_trace, verify_bitwise_xor_4_interaction_claim),
        (verify_bitwise_xor_7_trace, verify_bitwise_xor_7_interaction_claim),
        (verify_bitwise_xor_9_trace, verify_bitwise_xor_9_interaction_claim),
        (range_check_15_trace, range_check_15_interaction_claim),
        (range_check_16_trace, range_check_16_interaction_claim),
    ) = std::thread::scope(|s| {
        let eq_handle = s.spawn(|| {
            eq::write_interaction_trace(eq_log_size, eq_lookup_data, common_lookup_elements)
        });
        let qm31_ops_handle = s.spawn(|| {
        ...
        let blake_gate_handle =
            s.spawn(|| blake_gate.write_interaction_trace(common_lookup_elements));
        let blake_round_handle =
            s.spawn(|| blake_round.write_interaction_trace(common_lookup_elements));
        let blake_round_sigma_handle =
            s.spawn(|| blake_round_sigma.write_interaction_trace(common_lookup_elements));
        let blake_g_handle =
            s.spawn(|| blake_g.write_interaction_trace(common_lookup_elements));
        let blake_output_handle =
            s.spawn(|| blake_output.write_interaction_trace(common_lookup_elements));
        let triple_xor_32_handle =
            s.spawn(|| triple_xor_32.write_interaction_trace(common_lookup_elements));
        let verify_bitwise_xor_8_handle =
            s.spawn(|| verify_bitwise_xor_8.write_interaction_trace(common_lookup_elements));
        let verify_bitwise_xor_12_handle =
            s.spawn(|| verify_bitwise_xor_12.write_interaction_trace(common_lookup_elements));
        let verify_bitwise_xor_4_handle =
            s.spawn(|| verify_bitwise_xor_4.write_interaction_trace(common_lookup_elements));
        let verify_bitwise_xor_7_handle =
            s.spawn(|| verify_bitwise_xor_7.write_interaction_trace(common_lookup_elements));
        let verify_bitwise_xor_9_handle =
            s.spawn(|| verify_bitwise_xor_9.write_interaction_trace(common_lookup_elements));
        let range_check_15_handle =
            s.spawn(|| range_check_15.write_interaction_trace(common_lookup_elements));
        let range_check_16_handle =
            s.spawn(|| range_check_16.write_interaction_trace(common_lookup_elements));

        (
            eq_handle.join().expect("eq interaction trace task failed"),
            qm31_ops_handle.join().expect("qm31_ops interaction trace task failed"),
            blake_gate_handle.join().expect("blake_gate interaction trace task failed"),
            blake_round_handle.join().expect("blake_round interaction trace task failed"),
            blake_round_sigma_handle
                .join()
                .expect("blake_round_sigma interaction trace task failed"),
            blake_g_handle.join().expect("blake_g interaction trace task failed"),
            blake_output_handle.join().expect("blake_output interaction trace task failed"),
            triple_xor_32_handle.join().expect("triple_xor_32 interaction trace task failed"),
            verify_bitwise_xor_8_handle
                .join()
                .expect("verify_bitwise_xor_8 interaction trace task failed"),
            verify_bitwise_xor_12_handle
                .join()
                .expect("verify_bitwise_xor_12 interaction trace task failed"),
            verify_bitwise_xor_4_handle
                .join()
                .expect("verify_bitwise_xor_4 interaction trace task failed"),
            verify_bitwise_xor_7_handle
                .join()
                .expect("verify_bitwise_xor_7 interaction trace task failed"),
            verify_bitwise_xor_9_handle
                .join()
                .expect("verify_bitwise_xor_9 interaction trace task failed"),
            range_check_15_handle.join().expect("range_check_15 interaction trace task failed"),
            range_check_16_handle.join().expect("range_check_16 interaction trace task failed"),
        )
    });

    // Collect all evals in component order, then interpolate in a single parallel batch.
    let mut interaction_trace_evals = eq_trace;
    interaction_trace_evals.extend(qm31_ops_trace);
    interaction_trace_evals.extend(blake_gate_trace);
    interaction_trace_evals.extend(blake_round_trace);
    interaction_trace_evals.extend(blake_round_sigma_trace);
    interaction_trace_evals.extend(blake_g_trace);
    interaction_trace_evals.extend(blake_output_trace);
    interaction_trace_evals.extend(triple_xor_32_trace);
    interaction_trace_evals.extend(verify_bitwise_xor_8_trace);
    interaction_trace_evals.extend(verify_bitwise_xor_12_trace);
    interaction_trace_evals.extend(verify_bitwise_xor_4_trace);
    interaction_trace_evals.extend(verify_bitwise_xor_7_trace);
    interaction_trace_evals.extend(verify_bitwise_xor_9_trace);
    interaction_trace_evals.extend(range_check_15_trace);
    interaction_trace_evals.extend(range_check_16_trace);
