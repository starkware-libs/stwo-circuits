        ...
        || {
            std::thread::scope(|s| {
                let verify_bitwise_xor_8_handle =
                    s.spawn(|| verify_bitwise_xor_8::ClaimGenerator::new(preprocessed_trace.clone()));
                let verify_bitwise_xor_12_handle =
                    s.spawn(|| verify_bitwise_xor_12::ClaimGenerator::new(preprocessed_trace.clone()));
                let verify_bitwise_xor_4_handle =
                    s.spawn(|| verify_bitwise_xor_4::ClaimGenerator::new(preprocessed_trace.clone()));
                let verify_bitwise_xor_7_handle =
                    s.spawn(|| verify_bitwise_xor_7::ClaimGenerator::new(preprocessed_trace.clone()));
                let verify_bitwise_xor_9_handle =
                    s.spawn(|| verify_bitwise_xor_9::ClaimGenerator::new(preprocessed_trace.clone()));
                ...
                (
                    ...
                    range_check_16_handle
                        .join()
                        .expect("range_check_16 init task failed"),
                    range_check_15_handle
                        .join()
                        .expect("range_check_15 init task failed"),
                    triple_xor_32_handle
                        .join()
                        .expect("triple_xor_32 init task failed"),
                    blake_gate_handle.join().expect("blake_gate init task failed"),
                    blake_round_handle
                        .join()
                        .expect("blake_round init task failed"),
                    blake_round_sigma_handle
                        .join()
                        .expect("blake_round_sigma init task failed"),
