...
impl CircuitComponents {
    pub fn new(
        ...
        // A constructor closure per component, indexed by `ComponentList`. Each receives the
        // `TraceLocationAllocator`, its trace log size, and its claimed sum, allocates its
        // component's columns from the allocator, and returns the boxed component. (Components with
        // a statically fixed trace size ignore the passed log size.)
        //
        // The `TraceLocationAllocator` assigns each component a contiguous range of committed
        // columns in the order the constructors are called. Calling them in size-sorted order (see
        // `sorted_component_order`, matching `write_trace`'s commit order) therefore allocates the
        // columns in committed order, so the committed column layout coincides with the natural
        // component order and the verifier can skip the query-column sort.
        ...
        let components: Vec<Box<dyn ComponentProver<SimdBackend>>> =
            zip_eq(component_log_sizes.values(), constructors)
