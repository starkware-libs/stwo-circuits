...
impl CircuitComponents {
    pub fn new(
        ...
        // `constructors` is indexed by `ComponentList`, so zipping it with `component_log_sizes`
        // relies on the map iterating in that order.
        debug_assert!(
            component_log_sizes.keys().copied().eq(COMPONENT_NAMES),
            "component_log_sizes must iterate in ComponentList order",
        );
