...
const MULTIVERIFIER_PREPROCESSED_ROOT: [u32; 8] =
    [1156124963, 198659909, 1069506302, 193491135, 533803349, 2089270679, 231976630, 591996044];
...
fn test_verify_cairo_proof_and_multiverifier_proof() {
    ...
    let payload_to_hash: Vec<QM31> = chain!(output_preimage, output_preimage).collect();
    let hash_of_payloads = blake_qm31(&payload_to_hash, 16 * payload_to_hash.len());

    let multiverifier_of_two_cairo_input = MultiverifierInput {
        proof: multiverifier_proof,
        output_values: [hash_of_payloads.0, hash_of_payloads.1],
    ...
    let mut context = build_multiverifier_circuit::<QM31>(
        ...
        make_multiverifier_input(&proof, &public_data, &common_config.proof_config),
        &common_config,
    );
    pad_to_targets(&mut context, TARGET_PADDING_LOG_SIZES);
