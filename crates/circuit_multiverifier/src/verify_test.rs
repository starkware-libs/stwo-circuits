...
const PRIVACY_CAIRO_VERIFIER_PREPROCESSED_ROOT: [u32; 8] =
    [1493891091, 1863879972, 1296164237, 948026753, 1940209258, 1798431385, 216933605, 1352247130];
...
#[test]
#[ignore = "Run to generate new multiverifier constants."]
...
fn test_prove_multiverifier_of_two_cairo_subcircuits() {
    ...
    let mut multiverifier_context = build_multiverifier_circuit::<QM31>(
        build_cairo_input(&proof, &public_data, &shared_config.proof_config),
        build_cairo_input(&proof, &public_data, &shared_config.proof_config),
        &shared_config,
    );
