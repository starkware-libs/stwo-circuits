...
const PRIVACY_CAIRO_VERIFIER_PREPROCESSED_ROOT: [u32; 8] =
    [1708321388, 184071895, 1894113041, 1079335172, 1929095799, 1364948018, 21271309, 1068343276];
const MULTIVERIFIER_PREPROCESSED_ROOT: [u32; 8] =
    [1470607838, 758384934, 1498602131, 1896283142, 1383818795, 2132354054, 1766152605, 2120313226];
const MULTIVERIFIER_N_PREPROCESSED_COLUMNS: usize = 45;
const PRIVACY_CAIRO_VERIFIER_N_PREPROCESSED_COLUMNS: usize = MULTIVERIFIER_N_PREPROCESSED_COLUMNS;
...
fn test_regression_constants() {
    ...
    assert_eq!(
        ...
        pp_multiverifier.preprocessed_trace.ids().len()
    );
    assert_eq!(
...
fn build_cairo_input(proof: &Proof<QM31>, proof_config: &ProofConfig) -> MultiverifierInput<QM31> {
    MultiverifierInput {
...
fn test_cairo_proof_regression() {
    ...
    if std::env::var("FIX_PROOF").is_err() {
        let proof_config = ProofConfig::new(
            &all_circuit_components::<QM31>(),
            PRIVACY_CAIRO_VERIFIER_N_PREPROCESSED_COLUMNS,
            &PCS_CONFIG,
            INTERACTION_POW_BITS,
        );
...
fn test_prove_multiverifier_of_two_cairo_subcircuits() {
    ...
    let mut multiverifier_context = build_multiverifier_circuit::<QM31>(
        build_cairo_input(&proof, &shared_config.proof_config),
        build_cairo_input(&proof, &shared_config.proof_config),
        &shared_config,
    );
