...
pub struct MultiverifierInput<Value: IValue> {
    ...
    /// [`circuits::context::U_VAR_IDX`]).
    pub inner_output_values: [QM31; 2],
}
...
pub fn build_multiverifier_circuit<Value: IValue>(
    ...
    for multiverifier_input in [input0, input1] {
        let MultiverifierInput { proof, preprocessed_root, inner_output_values } =
            multiverifier_input;

        let circuit_config = CircuitConfig {
            config: shared_config.pcs_config,
            n_outputs: shared_config.n_outputs,
            preprocessed_column_log_sizes: shared_config.preprocessed_column_log_sizes.clone(),
            preprocessed_root,
        };
        let statement = CircuitStatement::new(&mut context, &circuit_config, &inner_output_values);
