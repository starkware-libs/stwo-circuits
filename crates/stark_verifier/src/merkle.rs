...
pub fn verify_merkle_path<Value: IValue>(
    ...
    // `HashValue`s and the recomputed root is compared word-for-word against the lossless
    // `root` bound by the Fiat-Shamir channel.
...
pub fn decommit_eval_domain_samples<Value: IValue>(
    ...
    for (trace_idx, root) in roots.iter().enumerate() {
        ...
        for query_idx in 0..n_queries {
            ...
            verify_merkle_path(context, leaf, &bits_for_query, root.clone(), auth_path);
        }
    }
}
