...
/// `column_log_sizes_for_sorted_traces` maps a trace index to the column log sizes used to sort
...
pub fn decommit_eval_domain_samples<Value: IValue>(
    context: &mut Context<Value>,
    n_queries: usize,
    column_log_sizes_for_sorted_traces: &HashMap<usize, Vec<Var>>,
    ...
    for (trace_idx, root) in roots.iter().enumerate() {
        ...
        let mut query_sorter = match column_log_sizes_for_sorted_traces.get(&trace_idx) {
