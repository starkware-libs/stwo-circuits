...
pub fn compute_fri_input(
    ...
    // Assert d = im(r.pt.y) != 0 for all OODS points r. This has two consequences:
    //
    // 1. Non-zero numerator coefficients: The coefficient of `query_value` in each quotient
    ...
    //    only at pt and conj(pt). Query points q are M31 points with im(q.y) = 0, so
