...
pub fn verify<Value: IValue>(
    ...
    // roots of (composition_poly - claimed) * vanishing. The composition polynomial uses a 2-way
    // split (COMPOSITION_LOG_SPLIT = 1), so its degree is at most 2 * 2^log_trace_size ≤ 2^31,
