...
pub fn verify<Value: IValue>(
    ...
    let relation_uses = check_relation_uses(context, statement, &component_sizes_bits);
    let unpacked_component_sizes = Simd::unpack(context, &component_sizes);
    statement.verify_claim(context, &enable_bits, &unpacked_component_sizes, &relation_uses);
