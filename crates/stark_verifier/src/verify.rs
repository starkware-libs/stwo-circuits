...
pub fn verify<Value: IValue>(
    ...
    let n_components = context.constant(qm31_from_u32s(n_components.try_into().unwrap(), 0, 0, 0));
...
fn column_log_sizes_by_trace(
    ...
    let n_components = config.component_infos.len();
    let mut column_log_sizes = [
        Vec::with_capacity(n_components),
        Vec::with_capacity(n_components),
    ];

    for (component_info, log_size) in izip!(
        &config.component_infos,
...
fn column_periodicity_sample_points(
    ...
    let mut periodicity_sample_points_per_column = Vec::with_capacity(config.component_infos.len());
    for (component_info, sample_point) in
        izip!(&config.component_infos, sample_points_per_component)
    {
        ...
        periodicity_sample_points_per_column
            .extend(vec![sample_point; n_interaction_columns]);
    }
    periodicity_sample_points_per_column
}
