...
pub struct ProofInfo {
    ...
    // FRI witness per query: one QM31 circle sibling.
...
impl ProofInfo {
    ...
    pub fn from_config(config: &ProofConfig) -> Self {
        ...
        let line_degree_log_ratio =
            config.fri.log_trace_size - 1 - config.fri.log_n_last_layer_coefs;
        let all_line_fold_steps =
            compute_all_line_fold_steps(line_degree_log_ratio, config.fri.line_fold_step);
        ...
        let fri_circle_siblings_per_query = SECURE_EXTENSION_DEGREE;
        let fri_line_coset_vals_per_query: usize = all_line_fold_steps
            .iter()
            .map(|step| (1 << step) * SECURE_EXTENSION_DEGREE)
            .sum();
...
impl std::fmt::Display for ProofInfo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  fri_circle_siblings: {:>10} = {fri_circle_siblings_per_query} * {q}",
            fri_circle_siblings_per_query * q,
        )?;
        writeln!(
            f,
