...
impl ProofConfig {
    ...
    pub fn new(
        ...
        let mut n_trace_columns: usize = 0;
        let mut n_interaction_columns: usize = 0;
        let mut cumulative_sum_columns = Vec::with_capacity(n_interaction_columns);
        for info in &component_shapes {
            n_trace_columns += info.trace_columns;
            n_interaction_columns += info.interaction_columns;
    ...
    pub fn n_interaction_columns(&self) -> usize {
        self.component_shapes.iter().map(|info| info.interaction_columns).sum()
    }
