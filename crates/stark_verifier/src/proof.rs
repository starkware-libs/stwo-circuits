...
pub struct ProofInfo {
    ...
    // FRI witness per query: circle-to-line fold coset values (QM31).
    pub fri_circle_fold_vals_per_query: usize,
    // Each line fold layer has 2^step QM31 coset values per query.
...
impl ProofInfo {
    ...
    pub fn from_config(config: &ProofConfig) -> Self {
        ...
        let packed_log_sizes = config.n_components.div_ceil(4); // 4 log per 
        ...
        // The number of cumulative sum columns that have to mask values per columns.
        ...
        let all_fold_steps =
            compute_all_fold_steps(degree_log_ratio, config.fri.fold_step);
        ...
        let fri_circle_fold_vals_per_query =
            (1 << all_fold_steps[0]) * SECURE_EXTENSION_DEGREE;
        let fri_line_coset_vals_per_query: usize =
            all_fold_steps[1..].iter().map(|step| (1 << step) * SECURE_EXTENSION_DEGREE).sum();
        ...
        Self {
            ...
            eval_auth_per_query,
            fri_auth_per_query,
            fri_circle_fold_vals_per_query,
...
impl std::fmt::Display for ProofInfo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  eval_samples:        {:>10} = {eval_samples_per_query} * {q}",
            eval_samples_per_query * q,
        )?;
        writeln!(
            f,
            "  eval_auth_paths:     {:>10} = {eval_auth_per_query} * {q}",
            eval_auth_per_query * q,
        )?;
        writeln!(f, "  fri_commitments:     {fri_commitments:>10}")?;
        writeln!(f, "  fri_last_layer:      {fri_last_layer:>10}")?;
        writeln!(
            f,
            "  fri_auth:            {:>10} = {fri_auth_per_query} * {q}",
            fri_auth_per_query * q,
        )?;
        writeln!(
            f,
            "  fri_circle_fold:     {:>10} = {fri_circle_fold_vals_per_query} * {q}",
            fri_circle_fold_vals_per_query * q,
        )?;
        writeln!(
            f,
            "  fri_line_coset_vals: {:>10} = {fri_line_coset_vals_per_query} * {q}",
            fri_line_coset_vals_per_query * q,
        )?;
        writeln!(f, "  ─────────────────────────────")?;
