...
fn verify_sorted_keys<Value: IValue>(context: &mut Context<Value>, sorted_keys: &[Var]) {
    ...
    let k = LOG_SIZE_BITS + column_idx_bits;
    // k is used for both the key width and the range check.
    // Keys lie in [0, 2**k - 1], so diffs lie in [-(2**k - 1), 2**k - 1].
    //
    // Positive diffs are accepted by construction. For a negative diff to be
    // rejected, its M31 representation must be >= 2**k. The most-negative diff
    // is -(2**k - 1), which wraps to 2**31 - 2**k, giving:
    //
    //   2**31 - 2**k >= 2**k
    //   => 2**31 - 2 * 2**k >= 0
    //   => k <= 30
    assert!(k <= 30);
    let _bits = extract_bits(context, &packed, k);
}
