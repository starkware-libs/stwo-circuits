...
pub(crate) fn finalize_constants_with_min_base(
    ...
    // TODO(dan): Consider adding the limbs from all cm31s and qm31s into find_max_conecutive.
    ...
    m31_cache.insert(M31(0), zero_idx);
    ...
    let u_val = qm31_from_u32s(0, 0, 1, 0);
    qm31_cache.insert(u_val, qm31_constants.swap_remove(&u_val).unwrap().idx);
...
fn add_cm31_constant(
    ...
    let a_plus_bi_idx = *qm31_cache.entry(a_plus_bi).or_insert_with(|| {
...
fn decompose_qm31_constants(
    ...
    while let Some(&qm31_val) = qm31_constants.keys().next() {
        let limbs = qm31_val.to_m31_array();
        let [a, b, c, d]: [u32; 4] = std::array::from_fn(|j| limbs[j].0);
