...
fn build_plus_one_chain(
    ...
    for val in 2..=m31_base {
        let var = swap_or_new(context, m31_constants, M31::from(val));
...
fn build_m31_from_base(
    ...
    for &limb in limbs.iter().rev() {
        ...
        let mul_idx = *m31_cache.entry(mul_val).or_insert_with(|| {
            let var = swap_or_new(context, m31_constants, mul_val);
        ...
        let add_idx = *m31_cache.entry(add_val).or_insert_with(|| {
            let var = swap_or_new(context, m31_constants, add_val);
...
fn decompose_broadcast_constants(
    ...
    let ones_var = swap_or_new(context, qm31_constants, ones_value);
...
/// we remove it from the list and it. Otherwise, we create and return a new variable.
fn swap_or_new<T>(
    context: &mut Context<impl IValue>,
