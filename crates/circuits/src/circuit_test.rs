use crate::circuit::{Add, Circuit, Eq, Mul, Output, Permutation, Sub, Var};
...
fn circuit_add() {
    ...
    c.add.push(Add { in0: var(0), in1: var(1), out: var(2) });
...
fn circuit_sub() {
    ...
    c.sub.push(Sub { in0: var(0), in1: var(1), out: var(3) });
...
fn circuit_mul() {
    let mut c = Circuit::default();
    c.mul.push(Mul { in0: var(1), in1: var(1), out: var(2) });
...
fn circuit_eq() {
    let mut c = Circuit { n_vars: 3, ..Circuit::default() };
    c.eq.push(Eq { in0: var(1), in1: var(2) });
...
fn circuit_permutation() {
    let mut c = Circuit { n_vars: 6, ..Circuit::default() };
    c.permutation.push(Permutation {
        inputs: vec![var(0), var(1), var(2)],
        outputs: vec![var(3), var(4), var(5)],
    });
    c.check(&[1.into(), 2.into(), 2.into(), 2.into(), 1.into(), 2.into()]).unwrap();
    assert_eq!(
...
fn circuit_output() {
    let mut c = Circuit { n_vars: 3, ..Circuit::default() };
    c.output.push(Output { in0: var(1) });
    c.check(&[1.into(), 2.into(), 2.into()]).unwrap();
