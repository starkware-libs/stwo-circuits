...
///  Committing and iterating the circuit components in this order makes the committed column layout
/// coincide with the natural component order, which lets the verifier set
/// `Statement::sorting_required` to `false` and skip the in-circuit query-column sort during
/// decommitment.
///
