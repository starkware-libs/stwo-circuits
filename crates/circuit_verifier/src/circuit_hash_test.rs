...
fn expected_circuit_hash(
    ...
    config_bytes
        .extend(COMPONENT_NAMES.iter().map(|name| *component_log_sizes.get(*name).unwrap() as u8));
    let config_words =
...
fn compute_circuit_hash_matches_expected() {
    ...
    // Both the in-circuit builder and the host twin must match the independent reference (the
    // invariant the Fiat-Shamir transcript relies on).
