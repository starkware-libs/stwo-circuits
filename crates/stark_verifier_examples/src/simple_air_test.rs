...
fn verify_simple_proof() {
    ...
    // The stwo prover mixed the full (padded) transcript, with one slot per component in the
    // full layout — including disabled components. Reconstruct that layout by padding the
    // filtered `claimed_sums` returned in the `Claim` with zeros for disabled slots.
    let mut claimed_sums_iter = claimed_sums.iter().copied();
    let padded_claimed_sums: Vec<QM31> = COMPONENT_ENABLE_BITS
        .iter()
        .map(|&enabled| if enabled { claimed_sums_iter.next().unwrap() } else { QM31::zero() })
        .collect();
    ...
    verifier_channel.mix_felts(&padded_claimed_sums);
