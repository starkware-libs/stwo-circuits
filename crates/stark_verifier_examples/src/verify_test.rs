...
fn test_verify(#[case] proof_modifier: ProofModifier) {
    ...
    match proof_modifier {
        ...
        ProofModifier::WrongFriWitness => {
            ...
            let shift: usize = fold_steps[..fold_steps.len() - 1].iter().sum();
            let queried_pos = first_query >> shift;
            let selected_idx = queried_pos & ((1 << last_step) - 1);
            let expected_value =
                context.get(proof_vars.fri.witness.0.last().unwrap()[0][selected_idx]);
            assert!(err.contains(&expected_value.to_string()));
        }
    }
