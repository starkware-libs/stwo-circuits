...
///
/// Note these limbs span `N_OUTPUT_VALUE_LIMBS * LIMB_BITS` bits (135 > 128), so a malicious
/// program could pack up to that many bits into each felt252 output; valid programs are known to
/// output u128s.
const N_OUTPUT_VALUE_LIMBS: usize = 128usize.div_ceil(LIMB_BITS);
