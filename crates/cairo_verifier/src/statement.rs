...
impl<Value: IValue> CairoStatement<Value> {
    /// `serialized_aux_data` is the serialized auxiliary data laid out as:
    /// `[aux_data (AUX_DATA_FIXED_LEN + outputs.len() + program.len() M31s) | component_log_sizes
    /// (components.len() M31s)]`.
    ///
...
impl<Value: IValue> Statement<Value> for CairoStatement<Value> {
    ...
    fn claims_to_mix(&self, context: &mut Context<Value>) -> Vec<Vec<U32Wrapper<Var>>> {
        ...
        .collect();
        let enable_bits_u32s: Vec<_> =
            enable_bit_vars.into_iter().map(|v| to_u32(context, v)).collect();
        ...
        let aux_data_u32s: Vec<_> = chain!(
            [context.constant((program.len() as u32).into())],
            [
