// This file was created by the AIR team.

use super::prelude::*;

pub const N_TRACE_COLUMNS: usize = 174;
pub const N_INTERACTION_COLUMNS: usize = 148;

pub const RELATION_USES_PER_ROW: [RelationUse; 8] = [
    RelationUse { relation_id: "BlakeRound", uses: 1 },
    RelationUse { relation_id: "MemoryAddressToId", uses: 20 },
    RelationUse { relation_id: "MemoryIdToBig", uses: 20 },
    RelationUse { relation_id: "Opcodes", uses: 1 },
    RelationUse { relation_id: "RangeCheck_7_2_5", uses: 17 },
    RelationUse { relation_id: "TripleXor32", uses: 8 },
    RelationUse { relation_id: "VerifyBitwiseXor_8", uses: 4 },
    RelationUse { relation_id: "VerifyInstruction", uses: 1 },
];

#[allow(unused_variables)]
pub fn accumulate_constraints<Value: IValue>(
    input: &[Var],
    context: &mut Context<Value>,
    component_data: &dyn ComponentDataTrait<Value>,
    acc: &mut CompositionConstraintAccumulator,
) {
    let [
        enabler_col0,
        input_pc_col1,
        input_ap_col2,
        input_fp_col3,
        offset0_col4,
        offset1_col5,
        offset2_col6,
        dst_base_fp_col7,
        op0_base_fp_col8,
        op1_base_fp_col9,
        ap_update_add_1_col10,
        opcode_extension_col11,
        mem0_base_col12,
        op0_id_col13,
        op0_limb_0_col14,
        op0_limb_1_col15,
        op0_limb_2_col16,
        op0_limb_3_col17,
        partial_limb_msb_col18,
        mem1_base_col19,
        op1_id_col20,
        op1_limb_0_col21,
        op1_limb_1_col22,
        op1_limb_2_col23,
        op1_limb_3_col24,
        partial_limb_msb_col25,
        ap_id_col26,
        ap_limb_0_col27,
        ap_limb_1_col28,
        ap_limb_2_col29,
        ap_limb_3_col30,
        partial_limb_msb_col31,
        mem_dst_base_col32,
        low_16_bits_col33,
        high_16_bits_col34,
        low_7_ms_bits_col35,
        high_14_ms_bits_col36,
        high_5_ms_bits_col37,
        dst_id_col38,
        low_16_bits_col39,
        high_16_bits_col40,
        low_7_ms_bits_col41,
        high_14_ms_bits_col42,
        high_5_ms_bits_col43,
        state_0_id_col44,
        low_16_bits_col45,
        high_16_bits_col46,
        low_7_ms_bits_col47,
        high_14_ms_bits_col48,
        high_5_ms_bits_col49,
        state_1_id_col50,
        low_16_bits_col51,
        high_16_bits_col52,
        low_7_ms_bits_col53,
        high_14_ms_bits_col54,
        high_5_ms_bits_col55,
        state_2_id_col56,
        low_16_bits_col57,
        high_16_bits_col58,
        low_7_ms_bits_col59,
        high_14_ms_bits_col60,
        high_5_ms_bits_col61,
        state_3_id_col62,
        low_16_bits_col63,
        high_16_bits_col64,
        low_7_ms_bits_col65,
        high_14_ms_bits_col66,
        high_5_ms_bits_col67,
        state_4_id_col68,
        low_16_bits_col69,
        high_16_bits_col70,
        low_7_ms_bits_col71,
        high_14_ms_bits_col72,
        high_5_ms_bits_col73,
        state_5_id_col74,
        low_16_bits_col75,
        high_16_bits_col76,
        low_7_ms_bits_col77,
        high_14_ms_bits_col78,
        high_5_ms_bits_col79,
        state_6_id_col80,
        low_16_bits_col81,
        high_16_bits_col82,
        low_7_ms_bits_col83,
        high_14_ms_bits_col84,
        high_5_ms_bits_col85,
        state_7_id_col86,
        ms_8_bits_col87,
        ms_8_bits_col88,
        xor_col89,
        xor_col90,
        xor_col91,
        xor_col92,
        blake_round_output_limb_0_col93,
        blake_round_output_limb_1_col94,
        blake_round_output_limb_2_col95,
        blake_round_output_limb_3_col96,
        blake_round_output_limb_4_col97,
        blake_round_output_limb_5_col98,
        blake_round_output_limb_6_col99,
        blake_round_output_limb_7_col100,
        blake_round_output_limb_8_col101,
        blake_round_output_limb_9_col102,
        blake_round_output_limb_10_col103,
        blake_round_output_limb_11_col104,
        blake_round_output_limb_12_col105,
        blake_round_output_limb_13_col106,
        blake_round_output_limb_14_col107,
        blake_round_output_limb_15_col108,
        blake_round_output_limb_16_col109,
        blake_round_output_limb_17_col110,
        blake_round_output_limb_18_col111,
        blake_round_output_limb_19_col112,
        blake_round_output_limb_20_col113,
        blake_round_output_limb_21_col114,
        blake_round_output_limb_22_col115,
        blake_round_output_limb_23_col116,
        blake_round_output_limb_24_col117,
        blake_round_output_limb_25_col118,
        blake_round_output_limb_26_col119,
        blake_round_output_limb_27_col120,
        blake_round_output_limb_28_col121,
        blake_round_output_limb_29_col122,
        blake_round_output_limb_30_col123,
        blake_round_output_limb_31_col124,
        blake_round_output_limb_32_col125,
        triple_xor_32_output_limb_0_col126,
        triple_xor_32_output_limb_1_col127,
        triple_xor_32_output_limb_0_col128,
        triple_xor_32_output_limb_1_col129,
        triple_xor_32_output_limb_0_col130,
        triple_xor_32_output_limb_1_col131,
        triple_xor_32_output_limb_0_col132,
        triple_xor_32_output_limb_1_col133,
        triple_xor_32_output_limb_0_col134,
        triple_xor_32_output_limb_1_col135,
        triple_xor_32_output_limb_0_col136,
        triple_xor_32_output_limb_1_col137,
        triple_xor_32_output_limb_0_col138,
        triple_xor_32_output_limb_1_col139,
        triple_xor_32_output_limb_0_col140,
        triple_xor_32_output_limb_1_col141,
        low_7_ms_bits_col142,
        high_14_ms_bits_col143,
        high_5_ms_bits_col144,
        new_state_0_id_col145,
        low_7_ms_bits_col146,
        high_14_ms_bits_col147,
        high_5_ms_bits_col148,
        new_state_1_id_col149,
        low_7_ms_bits_col150,
        high_14_ms_bits_col151,
        high_5_ms_bits_col152,
        new_state_2_id_col153,
        low_7_ms_bits_col154,
        high_14_ms_bits_col155,
        high_5_ms_bits_col156,
        new_state_3_id_col157,
        low_7_ms_bits_col158,
        high_14_ms_bits_col159,
        high_5_ms_bits_col160,
        new_state_4_id_col161,
        low_7_ms_bits_col162,
        high_14_ms_bits_col163,
        high_5_ms_bits_col164,
        new_state_5_id_col165,
        low_7_ms_bits_col166,
        high_14_ms_bits_col167,
        high_5_ms_bits_col168,
        new_state_6_id_col169,
        low_7_ms_bits_col170,
        high_14_ms_bits_col171,
        high_5_ms_bits_col172,
        new_state_7_id_col173,
    ] = input.try_into().unwrap();
    let seq = seq_of_component_size(context, component_data, &acc.preprocessed_columns);

    // Enabler is a bit.
    let constraint_0_value = eval!(context, ((enabler_col0) * (enabler_col0)) - (enabler_col0));
    acc.add_constraint(context, constraint_0_value);

    let [
        decode_blake_opcode_output_tmp_40cd9_42_limb_0,
        decode_blake_opcode_output_tmp_40cd9_42_limb_1,
        decode_blake_opcode_output_tmp_40cd9_42_limb_2,
        decode_blake_opcode_output_tmp_40cd9_42_limb_6,
    ] = decode_blake_opcode::accumulate_constraints(
        &[
            eval!(context, input_pc_col1),
            eval!(context, input_ap_col2),
            eval!(context, input_fp_col3),
            eval!(context, enabler_col0),
            eval!(context, offset0_col4),
            eval!(context, offset1_col5),
            eval!(context, offset2_col6),
            eval!(context, dst_base_fp_col7),
            eval!(context, op0_base_fp_col8),
            eval!(context, op1_base_fp_col9),
            eval!(context, ap_update_add_1_col10),
            eval!(context, opcode_extension_col11),
            eval!(context, mem0_base_col12),
            eval!(context, op0_id_col13),
            eval!(context, op0_limb_0_col14),
            eval!(context, op0_limb_1_col15),
            eval!(context, op0_limb_2_col16),
            eval!(context, op0_limb_3_col17),
            eval!(context, partial_limb_msb_col18),
            eval!(context, mem1_base_col19),
            eval!(context, op1_id_col20),
            eval!(context, op1_limb_0_col21),
            eval!(context, op1_limb_1_col22),
            eval!(context, op1_limb_2_col23),
            eval!(context, op1_limb_3_col24),
            eval!(context, partial_limb_msb_col25),
            eval!(context, ap_id_col26),
            eval!(context, ap_limb_0_col27),
            eval!(context, ap_limb_1_col28),
            eval!(context, ap_limb_2_col29),
            eval!(context, ap_limb_3_col30),
            eval!(context, partial_limb_msb_col31),
            eval!(context, mem_dst_base_col32),
            eval!(context, low_16_bits_col33),
            eval!(context, high_16_bits_col34),
            eval!(context, low_7_ms_bits_col35),
            eval!(context, high_14_ms_bits_col36),
            eval!(context, high_5_ms_bits_col37),
            eval!(context, dst_id_col38),
        ],
        context,
        component_data,
        acc,
    )
    .try_into()
    .unwrap();

    let [
        create_blake_round_input_output_tmp_40cd9_143_limb_24,
        create_blake_round_input_output_tmp_40cd9_143_limb_25,
        create_blake_round_input_output_tmp_40cd9_143_limb_28,
        create_blake_round_input_output_tmp_40cd9_143_limb_29,
    ] = create_blake_round_input::accumulate_constraints(
        &[
            eval!(context, decode_blake_opcode_output_tmp_40cd9_42_limb_0),
            eval!(context, low_16_bits_col33),
            eval!(context, high_16_bits_col34),
            eval!(context, decode_blake_opcode_output_tmp_40cd9_42_limb_6),
            eval!(context, enabler_col0),
            eval!(context, low_16_bits_col39),
            eval!(context, high_16_bits_col40),
            eval!(context, low_7_ms_bits_col41),
            eval!(context, high_14_ms_bits_col42),
            eval!(context, high_5_ms_bits_col43),
            eval!(context, state_0_id_col44),
            eval!(context, low_16_bits_col45),
            eval!(context, high_16_bits_col46),
            eval!(context, low_7_ms_bits_col47),
            eval!(context, high_14_ms_bits_col48),
            eval!(context, high_5_ms_bits_col49),
            eval!(context, state_1_id_col50),
            eval!(context, low_16_bits_col51),
            eval!(context, high_16_bits_col52),
            eval!(context, low_7_ms_bits_col53),
            eval!(context, high_14_ms_bits_col54),
            eval!(context, high_5_ms_bits_col55),
            eval!(context, state_2_id_col56),
            eval!(context, low_16_bits_col57),
            eval!(context, high_16_bits_col58),
            eval!(context, low_7_ms_bits_col59),
            eval!(context, high_14_ms_bits_col60),
            eval!(context, high_5_ms_bits_col61),
            eval!(context, state_3_id_col62),
            eval!(context, low_16_bits_col63),
            eval!(context, high_16_bits_col64),
            eval!(context, low_7_ms_bits_col65),
            eval!(context, high_14_ms_bits_col66),
            eval!(context, high_5_ms_bits_col67),
            eval!(context, state_4_id_col68),
            eval!(context, low_16_bits_col69),
            eval!(context, high_16_bits_col70),
            eval!(context, low_7_ms_bits_col71),
            eval!(context, high_14_ms_bits_col72),
            eval!(context, high_5_ms_bits_col73),
            eval!(context, state_5_id_col74),
            eval!(context, low_16_bits_col75),
            eval!(context, high_16_bits_col76),
            eval!(context, low_7_ms_bits_col77),
            eval!(context, high_14_ms_bits_col78),
            eval!(context, high_5_ms_bits_col79),
            eval!(context, state_6_id_col80),
            eval!(context, low_16_bits_col81),
            eval!(context, high_16_bits_col82),
            eval!(context, low_7_ms_bits_col83),
            eval!(context, high_14_ms_bits_col84),
            eval!(context, high_5_ms_bits_col85),
            eval!(context, state_7_id_col86),
            eval!(context, ms_8_bits_col87),
            eval!(context, ms_8_bits_col88),
            eval!(context, xor_col89),
            eval!(context, xor_col90),
            eval!(context, xor_col91),
            eval!(context, xor_col92),
        ],
        context,
        component_data,
        acc,
    )
    .try_into()
    .unwrap();

    // Yield BlakeRound.
    let tuple_3 = &[
        eval!(context, 40528774),
        eval!(context, seq),
        eval!(context, 0),
        eval!(context, low_16_bits_col39),
        eval!(context, high_16_bits_col40),
        eval!(context, low_16_bits_col45),
        eval!(context, high_16_bits_col46),
        eval!(context, low_16_bits_col51),
        eval!(context, high_16_bits_col52),
        eval!(context, low_16_bits_col57),
        eval!(context, high_16_bits_col58),
        eval!(context, low_16_bits_col63),
        eval!(context, high_16_bits_col64),
        eval!(context, low_16_bits_col69),
        eval!(context, high_16_bits_col70),
        eval!(context, low_16_bits_col75),
        eval!(context, high_16_bits_col76),
        eval!(context, low_16_bits_col81),
        eval!(context, high_16_bits_col82),
        eval!(context, 58983),
        eval!(context, 27145),
        eval!(context, 44677),
        eval!(context, 47975),
        eval!(context, 62322),
        eval!(context, 15470),
        eval!(context, 62778),
        eval!(context, 42319),
        eval!(context, create_blake_round_input_output_tmp_40cd9_143_limb_24),
        eval!(context, create_blake_round_input_output_tmp_40cd9_143_limb_25),
        eval!(context, 26764),
        eval!(context, 39685),
        eval!(context, create_blake_round_input_output_tmp_40cd9_143_limb_28),
        eval!(context, create_blake_round_input_output_tmp_40cd9_143_limb_29),
        eval!(context, 52505),
        eval!(context, 23520),
        eval!(context, decode_blake_opcode_output_tmp_40cd9_42_limb_1),
    ];
    let numerator_3 = eval!(context, -(enabler_col0));
    acc.add_to_relation(context, numerator_3, tuple_3);

    // Use BlakeRound.
    let tuple_4 = &[
        eval!(context, 40528774),
        eval!(context, seq),
        eval!(context, 10),
        eval!(context, blake_round_output_limb_0_col93),
        eval!(context, blake_round_output_limb_1_col94),
        eval!(context, blake_round_output_limb_2_col95),
        eval!(context, blake_round_output_limb_3_col96),
        eval!(context, blake_round_output_limb_4_col97),
        eval!(context, blake_round_output_limb_5_col98),
        eval!(context, blake_round_output_limb_6_col99),
        eval!(context, blake_round_output_limb_7_col100),
        eval!(context, blake_round_output_limb_8_col101),
        eval!(context, blake_round_output_limb_9_col102),
        eval!(context, blake_round_output_limb_10_col103),
        eval!(context, blake_round_output_limb_11_col104),
        eval!(context, blake_round_output_limb_12_col105),
        eval!(context, blake_round_output_limb_13_col106),
        eval!(context, blake_round_output_limb_14_col107),
        eval!(context, blake_round_output_limb_15_col108),
        eval!(context, blake_round_output_limb_16_col109),
        eval!(context, blake_round_output_limb_17_col110),
        eval!(context, blake_round_output_limb_18_col111),
        eval!(context, blake_round_output_limb_19_col112),
        eval!(context, blake_round_output_limb_20_col113),
        eval!(context, blake_round_output_limb_21_col114),
        eval!(context, blake_round_output_limb_22_col115),
        eval!(context, blake_round_output_limb_23_col116),
        eval!(context, blake_round_output_limb_24_col117),
        eval!(context, blake_round_output_limb_25_col118),
        eval!(context, blake_round_output_limb_26_col119),
        eval!(context, blake_round_output_limb_27_col120),
        eval!(context, blake_round_output_limb_28_col121),
        eval!(context, blake_round_output_limb_29_col122),
        eval!(context, blake_round_output_limb_30_col123),
        eval!(context, blake_round_output_limb_31_col124),
        eval!(context, blake_round_output_limb_32_col125),
    ];
    let numerator_4 = eval!(context, enabler_col0);
    acc.add_to_relation(context, numerator_4, tuple_4);

    create_blake_output::accumulate_constraints(
        &[
            eval!(context, low_16_bits_col39),
            eval!(context, high_16_bits_col40),
            eval!(context, low_16_bits_col45),
            eval!(context, high_16_bits_col46),
            eval!(context, low_16_bits_col51),
            eval!(context, high_16_bits_col52),
            eval!(context, low_16_bits_col57),
            eval!(context, high_16_bits_col58),
            eval!(context, low_16_bits_col63),
            eval!(context, high_16_bits_col64),
            eval!(context, low_16_bits_col69),
            eval!(context, high_16_bits_col70),
            eval!(context, low_16_bits_col75),
            eval!(context, high_16_bits_col76),
            eval!(context, low_16_bits_col81),
            eval!(context, high_16_bits_col82),
            eval!(context, blake_round_output_limb_0_col93),
            eval!(context, blake_round_output_limb_1_col94),
            eval!(context, blake_round_output_limb_2_col95),
            eval!(context, blake_round_output_limb_3_col96),
            eval!(context, blake_round_output_limb_4_col97),
            eval!(context, blake_round_output_limb_5_col98),
            eval!(context, blake_round_output_limb_6_col99),
            eval!(context, blake_round_output_limb_7_col100),
            eval!(context, blake_round_output_limb_8_col101),
            eval!(context, blake_round_output_limb_9_col102),
            eval!(context, blake_round_output_limb_10_col103),
            eval!(context, blake_round_output_limb_11_col104),
            eval!(context, blake_round_output_limb_12_col105),
            eval!(context, blake_round_output_limb_13_col106),
            eval!(context, blake_round_output_limb_14_col107),
            eval!(context, blake_round_output_limb_15_col108),
            eval!(context, blake_round_output_limb_16_col109),
            eval!(context, blake_round_output_limb_17_col110),
            eval!(context, blake_round_output_limb_18_col111),
            eval!(context, blake_round_output_limb_19_col112),
            eval!(context, blake_round_output_limb_20_col113),
            eval!(context, blake_round_output_limb_21_col114),
            eval!(context, blake_round_output_limb_22_col115),
            eval!(context, blake_round_output_limb_23_col116),
            eval!(context, blake_round_output_limb_24_col117),
            eval!(context, blake_round_output_limb_25_col118),
            eval!(context, blake_round_output_limb_26_col119),
            eval!(context, blake_round_output_limb_27_col120),
            eval!(context, blake_round_output_limb_28_col121),
            eval!(context, blake_round_output_limb_29_col122),
            eval!(context, blake_round_output_limb_30_col123),
            eval!(context, blake_round_output_limb_31_col124),
            eval!(context, enabler_col0),
            eval!(context, triple_xor_32_output_limb_0_col126),
            eval!(context, triple_xor_32_output_limb_1_col127),
            eval!(context, triple_xor_32_output_limb_0_col128),
            eval!(context, triple_xor_32_output_limb_1_col129),
            eval!(context, triple_xor_32_output_limb_0_col130),
            eval!(context, triple_xor_32_output_limb_1_col131),
            eval!(context, triple_xor_32_output_limb_0_col132),
            eval!(context, triple_xor_32_output_limb_1_col133),
            eval!(context, triple_xor_32_output_limb_0_col134),
            eval!(context, triple_xor_32_output_limb_1_col135),
            eval!(context, triple_xor_32_output_limb_0_col136),
            eval!(context, triple_xor_32_output_limb_1_col137),
            eval!(context, triple_xor_32_output_limb_0_col138),
            eval!(context, triple_xor_32_output_limb_1_col139),
            eval!(context, triple_xor_32_output_limb_0_col140),
            eval!(context, triple_xor_32_output_limb_1_col141),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, decode_blake_opcode_output_tmp_40cd9_42_limb_2),
            eval!(context, triple_xor_32_output_limb_0_col126),
            eval!(context, triple_xor_32_output_limb_1_col127),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col142),
            eval!(context, high_14_ms_bits_col143),
            eval!(context, high_5_ms_bits_col144),
            eval!(context, new_state_0_id_col145),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (1)),
            eval!(context, triple_xor_32_output_limb_0_col128),
            eval!(context, triple_xor_32_output_limb_1_col129),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col146),
            eval!(context, high_14_ms_bits_col147),
            eval!(context, high_5_ms_bits_col148),
            eval!(context, new_state_1_id_col149),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (2)),
            eval!(context, triple_xor_32_output_limb_0_col130),
            eval!(context, triple_xor_32_output_limb_1_col131),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col150),
            eval!(context, high_14_ms_bits_col151),
            eval!(context, high_5_ms_bits_col152),
            eval!(context, new_state_2_id_col153),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (3)),
            eval!(context, triple_xor_32_output_limb_0_col132),
            eval!(context, triple_xor_32_output_limb_1_col133),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col154),
            eval!(context, high_14_ms_bits_col155),
            eval!(context, high_5_ms_bits_col156),
            eval!(context, new_state_3_id_col157),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (4)),
            eval!(context, triple_xor_32_output_limb_0_col134),
            eval!(context, triple_xor_32_output_limb_1_col135),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col158),
            eval!(context, high_14_ms_bits_col159),
            eval!(context, high_5_ms_bits_col160),
            eval!(context, new_state_4_id_col161),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (5)),
            eval!(context, triple_xor_32_output_limb_0_col136),
            eval!(context, triple_xor_32_output_limb_1_col137),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col162),
            eval!(context, high_14_ms_bits_col163),
            eval!(context, high_5_ms_bits_col164),
            eval!(context, new_state_5_id_col165),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (6)),
            eval!(context, triple_xor_32_output_limb_0_col138),
            eval!(context, triple_xor_32_output_limb_1_col139),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col166),
            eval!(context, high_14_ms_bits_col167),
            eval!(context, high_5_ms_bits_col168),
            eval!(context, new_state_6_id_col169),
        ],
        context,
        component_data,
        acc,
    );

    verify_u_32::accumulate_constraints(
        &[
            eval!(context, (decode_blake_opcode_output_tmp_40cd9_42_limb_2) + (7)),
            eval!(context, triple_xor_32_output_limb_0_col140),
            eval!(context, triple_xor_32_output_limb_1_col141),
            eval!(context, enabler_col0),
            eval!(context, low_7_ms_bits_col170),
            eval!(context, high_14_ms_bits_col171),
            eval!(context, high_5_ms_bits_col172),
            eval!(context, new_state_7_id_col173),
        ],
        context,
        component_data,
        acc,
    );

    // Use Opcodes.
    let tuple_14 = &[
        eval!(context, 428564188),
        eval!(context, input_pc_col1),
        eval!(context, input_ap_col2),
        eval!(context, input_fp_col3),
    ];
    let numerator_14 = eval!(context, enabler_col0);
    acc.add_to_relation(context, numerator_14, tuple_14);

    // Yield Opcodes.
    let tuple_15 = &[
        eval!(context, 428564188),
        eval!(context, (input_pc_col1) + (1)),
        eval!(context, (input_ap_col2) + (ap_update_add_1_col10)),
        eval!(context, input_fp_col3),
    ];
    let numerator_15 = eval!(context, -(enabler_col0));
    acc.add_to_relation(context, numerator_15, tuple_15);
}

pub struct Component {}
impl<Value: IValue> CircuitEval<Value> for Component {
    fn name(&self) -> String {
        "blake_compress_opcode".to_string()
    }

    fn evaluate(
        &self,
        context: &mut Context<Value>,
        component_data: &dyn ComponentDataTrait<Value>,
        acc: &mut CompositionConstraintAccumulator,
    ) {
        accumulate_constraints(component_data.trace_columns(), context, component_data, acc);
    }

    fn trace_columns(&self) -> usize {
        N_TRACE_COLUMNS
    }

    fn interaction_columns(&self) -> usize {
        N_INTERACTION_COLUMNS
    }

    fn relation_uses_per_row(&self) -> &[RelationUse] {
        &RELATION_USES_PER_ROW
    }

    fn log_size(
        &self,
        _preprocessed_column_log_sizes: &OrderedHashMap<PreProcessedColumnId, u32>,
    ) -> Option<u32> {
        None
    }
}
#[cfg(test)]
mod tests {
    use std::collections::HashMap;

    use circuits::context::Context;
    use circuits::ivalue::qm31_from_u32s;
    use circuits_stark_verifier::constraint_eval::*;
    use circuits_stark_verifier::test_utils::TestComponentData;
    use stwo::core::fields::qm31::QM31;

    use super::Component;
    #[allow(unused_imports)]
    use crate::components::prelude::PreProcessedColumnId;
    use crate::sample_evaluations::*;

    #[test]
    fn test_evaluation_result() {
        let component = Component {};
        let mut context: Context<QM31> = Default::default();
        context.enable_assert_eq_on_eval();
        let trace_columns = [
            qm31_from_u32s(1659099300, 905558730, 651199673, 1375009625),
            qm31_from_u32s(1591990121, 771341002, 584090809, 1375009625),
            qm31_from_u32s(1793317658, 1173994186, 785417401, 1375009625),
            qm31_from_u32s(1726208479, 1039776458, 718308537, 1375009625),
            qm31_from_u32s(1390662584, 368687818, 382764217, 1375009625),
            qm31_from_u32s(1323553405, 234470090, 315655353, 1375009625),
            qm31_from_u32s(1524880942, 637123274, 516981945, 1375009625),
            qm31_from_u32s(1457771763, 502905546, 449873081, 1375009625),
            qm31_from_u32s(48489085, 1979300555, 1188070585, 1375009625),
            qm31_from_u32s(2128863553, 1845082826, 1120961721, 1375009625),
            qm31_from_u32s(1852335767, 645078115, 2059236183, 343880121),
            qm31_from_u32s(1919444946, 779295843, 2126345047, 343880121),
            qm31_from_u32s(1986554125, 913513571, 45970264, 343880122),
            qm31_from_u32s(2053663304, 1047731299, 113079128, 343880122),
            qm31_from_u32s(1583899051, 108207203, 1790800727, 343880121),
            qm31_from_u32s(1651008230, 242424931, 1857909591, 343880121),
            qm31_from_u32s(1718117409, 376642659, 1925018455, 343880121),
            qm31_from_u32s(1785226588, 510860387, 1992127319, 343880121),
            qm31_from_u32s(1315462335, 1718819938, 1522365270, 343880121),
            qm31_from_u32s(1382571514, 1853037666, 1589474134, 343880121),
            qm31_from_u32s(1986820986, 913513739, 45970432, 343880178),
            qm31_from_u32s(1919711807, 779296011, 2126345215, 343880177),
            qm31_from_u32s(2121039344, 1181949195, 180188160, 343880178),
            qm31_from_u32s(2053930165, 1047731467, 113079296, 343880178),
            qm31_from_u32s(1718384270, 376642827, 1925018623, 343880177),
            qm31_from_u32s(1651275091, 242425099, 1857909759, 343880177),
            qm31_from_u32s(1852602628, 645078283, 2059236351, 343880177),
            qm31_from_u32s(1785493449, 510860555, 1992127487, 343880177),
            qm31_from_u32s(1449947554, 1987255562, 1656583166, 343880177),
            qm31_from_u32s(1382838375, 1853037834, 1589474302, 343880177),
            qm31_from_u32s(510356977, 108207322, 717059022, 343880161),
            qm31_from_u32s(577466156, 242425050, 784167886, 343880161),
            qm31_from_u32s(376138619, 1987255513, 582841293, 343880161),
            qm31_from_u32s(443247798, 2121473241, 649950157, 343880161),
            qm31_from_u32s(778793693, 645078234, 985494478, 343880161),
            qm31_from_u32s(845902872, 779295962, 1052603342, 343880161),
            qm31_from_u32s(644575335, 376642778, 851276750, 343880161),
            qm31_from_u32s(711684514, 510860506, 918385614, 343880161),
            qm31_from_u32s(1047230409, 1181949146, 1253929934, 343880161),
            qm31_from_u32s(1114339588, 1316166874, 1321038798, 343880161),
            qm31_from_u32s(1717810224, 376642479, 1925018275, 343880061),
            qm31_from_u32s(1650701045, 242424751, 1857909411, 343880061),
            qm31_from_u32s(1583591866, 108207023, 1790800547, 343880061),
            qm31_from_u32s(1516482687, 2121472942, 1723691682, 343880061),
            qm31_from_u32s(1986246940, 913513391, 45970084, 343880062),
            qm31_from_u32s(1919137761, 779295663, 2126344867, 343880061),
            qm31_from_u32s(1852028582, 645077935, 2059236003, 343880061),
            qm31_from_u32s(1784919403, 510860207, 1992127139, 343880061),
            qm31_from_u32s(1180936792, 1450384302, 1388147362, 343880061),
            qm31_from_u32s(1113827613, 1316166574, 1321038498, 343880061),
            qm31_from_u32s(241305891, 1718819697, 448623205, 343880041),
            qm31_from_u32s(308415070, 1853037425, 515732069, 343880041),
            qm31_from_u32s(375524249, 1987255153, 582840933, 343880041),
            qm31_from_u32s(442633428, 2121472881, 649949797, 343880041),
            qm31_from_u32s(509742607, 108206962, 717058662, 343880041),
            qm31_from_u32s(576851786, 242424690, 784167526, 343880041),
            qm31_from_u32s(643960965, 376642418, 851276390, 343880041),
            qm31_from_u32s(711070144, 510860146, 918385254, 343880041),
            qm31_from_u32s(778179323, 645077874, 985494118, 343880041),
            qm31_from_u32s(845288502, 779295602, 1052602982, 343880041),
            qm31_from_u32s(375831434, 1987255333, 582841113, 343880101),
            qm31_from_u32s(308722255, 1853037605, 515732249, 343880101),
            qm31_from_u32s(510049792, 108207142, 717058842, 343880101),
            qm31_from_u32s(442940613, 2121473061, 649949977, 343880101),
            qm31_from_u32s(644268150, 376642598, 851276570, 343880101),
            qm31_from_u32s(577158971, 242424870, 784167706, 343880101),
            qm31_from_u32s(778486508, 645078054, 985494298, 343880101),
            qm31_from_u32s(711377329, 510860326, 918385434, 343880101),
            qm31_from_u32s(912704866, 913513510, 1119712026, 343880101),
            qm31_from_u32s(845595687, 779295782, 1052603162, 343880101),
            qm31_from_u32s(1046820829, 1181948906, 1253929694, 343880081),
            qm31_from_u32s(1113930008, 1316166634, 1321038558, 343880081),
            qm31_from_u32s(912602471, 913513450, 1119711966, 343880081),
            qm31_from_u32s(979711650, 1047731178, 1186820830, 343880081),
            qm31_from_u32s(778384113, 645077994, 985494238, 343880081),
            qm31_from_u32s(845493292, 779295722, 1052603102, 343880081),
            qm31_from_u32s(644165755, 376642538, 851276510, 343880081),
            qm31_from_u32s(711274934, 510860266, 918385374, 343880081),
            qm31_from_u32s(1583694261, 108207083, 1790800607, 343880081),
            qm31_from_u32s(1650803440, 242424811, 1857909471, 343880081),
            qm31_from_u32s(108388425, 1450385012, 314406248, 343880298),
            qm31_from_u32s(41279246, 1316167284, 247297384, 343880298),
            qm31_from_u32s(2121653714, 1181949555, 180188520, 343880298),
            qm31_from_u32s(2054544535, 1047731827, 113079656, 343880298),
            qm31_from_u32s(1987435356, 913514099, 45970792, 343880298),
            qm31_from_u32s(1920326177, 779296371, 2126345575, 343880297),
            qm31_from_u32s(1853216998, 645078643, 2059236711, 343880297),
            qm31_from_u32s(1786107819, 510860915, 1992127847, 343880297),
            qm31_from_u32s(1718998640, 376643187, 1925018983, 343880297),
            qm31_from_u32s(1651889461, 242425459, 1857910119, 343880297),
            qm31_from_u32s(779367739, 645078582, 985494826, 343880277),
            qm31_from_u32s(846476918, 779296310, 1052603690, 343880277),
            qm31_from_u32s(913586097, 913514038, 1119712554, 343880277),
            qm31_from_u32s(980695276, 1047731766, 1186821418, 343880277),
            qm31_from_u32s(510931023, 108207670, 717059370, 343880277),
            qm31_from_u32s(578040202, 242425398, 784168234, 343880277),
            qm31_from_u32s(645149381, 376643126, 851277098, 343880277),
            qm31_from_u32s(712258560, 510860854, 918385962, 343880277),
            qm31_from_u32s(1316241171, 1718820406, 1522365738, 343880277),
            qm31_from_u32s(1383350350, 1853038134, 1589474602, 343880277),
            qm31_from_u32s(1340598866, 536394231, 1198633759, 502514173),
            qm31_from_u32s(1407708045, 670611959, 1265742623, 502514173),
            qm31_from_u32s(1474817224, 804829687, 1332851487, 502514173),
            qm31_from_u32s(1541926403, 939047415, 1399960351, 502514173),
            qm31_from_u32s(1072162150, 2147006966, 930198302, 502514173),
            qm31_from_u32s(1139271329, 133741047, 997307167, 502514173),
            qm31_from_u32s(1206380508, 267958775, 1064416031, 502514173),
            qm31_from_u32s(1273489687, 402176503, 1131524895, 502514173),
            qm31_from_u32s(1877472298, 1610136055, 1735504671, 502514173),
            qm31_from_u32s(1944581477, 1744353783, 1802613535, 502514173),
            qm31_from_u32s(669619552, 1341700661, 527545181, 502514194),
            qm31_from_u32s(602510373, 1207482933, 460436317, 502514194),
            qm31_from_u32s(535401194, 1073265205, 393327453, 502514194),
            qm31_from_u32s(468292015, 939047477, 326218589, 502514194),
            qm31_from_u32s(401182836, 804829749, 259109725, 502514194),
            qm31_from_u32s(334073657, 670612021, 192000861, 502514194),
            qm31_from_u32s(266964478, 536394293, 124891997, 502514194),
            qm31_from_u32s(199855299, 402176565, 57783133, 502514194),
            qm31_from_u32s(132746120, 267958837, 2138157916, 502514193),
            qm31_from_u32s(65636941, 133741109, 2071049052, 502514193),
            qm31_from_u32s(2146113804, 2147007087, 2003940247, 502514213),
            qm31_from_u32s(65739336, 133741169, 2071049112, 502514213),
            qm31_from_u32s(2011895446, 1878571631, 1869722519, 502514213),
            qm31_from_u32s(2079004625, 2012789359, 1936831383, 502514213),
            qm31_from_u32s(267066873, 536394353, 124892057, 502514214),
            qm31_from_u32s(334176052, 670612081, 192000921, 502514214),
            qm31_from_u32s(132848515, 267958897, 2138157976, 502514213),
            qm31_from_u32s(199957694, 402176625, 57783193, 502514214),
            qm31_from_u32s(1609240372, 1073265263, 1467069335, 502514213),
            qm31_from_u32s(1676349551, 1207482991, 1534178199, 502514213),
            qm31_from_u32s(1475124409, 804829867, 1332851667, 502514233),
            qm31_from_u32s(1408015230, 670612139, 1265742803, 502514233),
            qm31_from_u32s(1609342767, 1073265323, 1467069395, 502514233),
            qm31_from_u32s(1542233588, 939047595, 1399960531, 502514233),
            qm31_from_u32s(1206687693, 267958955, 1064416211, 502514233),
            qm31_from_u32s(1139578514, 133741227, 997307347, 502514233),
            qm31_from_u32s(1340906051, 536394411, 1198633939, 502514233),
            qm31_from_u32s(1273796872, 402176683, 1131525075, 502514233),
            qm31_from_u32s(2011997841, 1878571691, 1869722579, 502514233),
            qm31_from_u32s(1944888662, 1744353963, 1802613715, 502514233),
            qm31_from_u32s(1877062718, 1610135815, 1735504431, 502514093),
            qm31_from_u32s(1944171897, 1744353543, 1802613295, 502514093),
            qm31_from_u32s(2011281076, 1878571271, 1869722159, 502514093),
            qm31_from_u32s(2078390255, 2012788999, 1936831023, 502514093),
            qm31_from_u32s(2145499434, 2147006727, 2003939887, 502514093),
            qm31_from_u32s(65124966, 133740809, 2071048752, 502514093),
            qm31_from_u32s(132234145, 267958537, 2138157616, 502514093),
            qm31_from_u32s(199343324, 402176265, 57782833, 502514094),
            qm31_from_u32s(1340189286, 536393991, 1198633519, 502514093),
            qm31_from_u32s(1407298465, 670611719, 1265742383, 502514093),
            qm31_from_u32s(1206073323, 267958595, 1064415851, 502514113),
            qm31_from_u32s(1138964144, 133740867, 997306987, 502514113),
            qm31_from_u32s(1071854965, 2147006786, 930198122, 502514113),
            qm31_from_u32s(1004745786, 2012789058, 863089258, 502514113),
            qm31_from_u32s(1474510039, 804829507, 1332851307, 502514113),
            qm31_from_u32s(1407400860, 670611779, 1265742443, 502514113),
            qm31_from_u32s(1340291681, 536394051, 1198633579, 502514113),
            qm31_from_u32s(1273182502, 402176323, 1131524715, 502514113),
            qm31_from_u32s(1742946755, 1341700419, 1601286763, 502514113),
            qm31_from_u32s(1675837576, 1207482691, 1534177899, 502514113),
            qm31_from_u32s(535094009, 1073265025, 393327273, 502514134),
            qm31_from_u32s(602203188, 1207482753, 460436137, 502514134),
            qm31_from_u32s(400875651, 804829569, 259109545, 502514134),
            qm31_from_u32s(467984830, 939047297, 326218409, 502514134),
            qm31_from_u32s(266657293, 536394113, 124891817, 502514134),
            qm31_from_u32s(333766472, 670611841, 192000681, 502514134),
            qm31_from_u32s(132438935, 267958657, 2138157736, 502514133),
            qm31_from_u32s(199548114, 402176385, 57782953, 502514134),
            qm31_from_u32s(2145704224, 2147006847, 2003940007, 502514133),
            qm31_from_u32s(65329756, 133740929, 2071048872, 502514133),
            qm31_from_u32s(2011588261, 1878571451, 1869722339, 502514153),
            qm31_from_u32s(1944479082, 1744353723, 1802613475, 502514153),
            qm31_from_u32s(2145806619, 2147006907, 2003940067, 502514153),
            qm31_from_u32s(2078697440, 2012789179, 1936831203, 502514153),
        ];
        let interaction_columns = [
            qm31_from_u32s(1005168032, 79980996, 1847888101, 1941984119),
            qm31_from_u32s(1072277211, 214198724, 1914996965, 1941984119),
            qm31_from_u32s(1139386390, 348416452, 1982105829, 1941984119),
            qm31_from_u32s(1206495569, 482634180, 2049214693, 1941984119),
            qm31_from_u32s(736731316, 1690593731, 1579452644, 1941984119),
            qm31_from_u32s(803840495, 1824811459, 1646561508, 1941984119),
            qm31_from_u32s(870949674, 1959029187, 1713670372, 1941984119),
            qm31_from_u32s(938058853, 2093246915, 1780779236, 1941984119),
            qm31_from_u32s(1542041464, 1153722820, 237275366, 1941984120),
            qm31_from_u32s(1609150643, 1287940548, 304384230, 1941984120),
            qm31_from_u32s(1577898798, 106101108, 1738096752, 1261630210),
            qm31_from_u32s(1510789619, 2119367027, 1670987887, 1261630210),
            qm31_from_u32s(1443680440, 1985149299, 1603879023, 1261630210),
            qm31_from_u32s(1376571261, 1850931571, 1536770159, 1261630210),
            qm31_from_u32s(1309462082, 1716713843, 1469661295, 1261630210),
            qm31_from_u32s(1242352903, 1582496115, 1402552431, 1261630210),
            qm31_from_u32s(1175243724, 1448278387, 1335443567, 1261630210),
            qm31_from_u32s(1108134545, 1314060659, 1268334703, 1261630210),
            qm31_from_u32s(2114772230, 1179842932, 127484017, 1261630211),
            qm31_from_u32s(2047663051, 1045625204, 60375153, 1261630211),
            qm31_from_u32s(906909403, 911407535, 1067008171, 1261630230),
            qm31_from_u32s(974018582, 1045625263, 1134117035, 1261630230),
            qm31_from_u32s(772691045, 642972079, 932790443, 1261630230),
            qm31_from_u32s(839800224, 777189807, 999899307, 1261630230),
            qm31_from_u32s(1175346119, 1448278447, 1335443627, 1261630230),
            qm31_from_u32s(1242455298, 1582496175, 1402552491, 1261630230),
            qm31_from_u32s(1041127761, 1179842991, 1201225899, 1261630230),
            qm31_from_u32s(1108236940, 1314060719, 1268334763, 1261630230),
            qm31_from_u32s(1443782835, 1985149359, 1603879083, 1261630230),
            qm31_from_u32s(1510892014, 2119367087, 1670987947, 1261630230),
            qm31_from_u32s(235889765, 1716713953, 395919581, 1261630247),
            qm31_from_u32s(168780586, 1582496225, 328810717, 1261630247),
            qm31_from_u32s(370108123, 1985149409, 530137309, 1261630247),
            qm31_from_u32s(302998944, 1850931681, 463028445, 1261630247),
            qm31_from_u32s(2114936696, 1179843040, 127484125, 1261630247),
            qm31_from_u32s(2047827517, 1045625312, 60375261, 1261630247),
            qm31_from_u32s(101671407, 1448278497, 261701853, 1261630247),
        ];
        let component_data = TestComponentData::from_values(
            &mut context,
            &trace_columns,
            &interaction_columns,
            qm31_from_u32s(1115374022, 1127856551, 489657863, 643630026),
            32768,
        );
        let random_coeff =
            context.new_var(qm31_from_u32s(474642921, 876336632, 1911695779, 974600512));
        let interaction_elements = [
            context.new_var(qm31_from_u32s(445623802, 202571636, 1360224996, 131355117)),
            context.new_var(qm31_from_u32s(476823935, 939223384, 62486082, 122423602)),
        ];
        let preprocessed_columns = HashMap::from([(
            PreProcessedColumnId { id: "seq_15".to_owned() },
            context.constant(qm31_from_u32s(735272696, 1215403647, 795393303, 879304430)),
        )]);
        let public_params = HashMap::from([]);
        let mut accumulator = CompositionConstraintAccumulator::new(
            &mut context,
            preprocessed_columns,
            public_params,
            random_coeff,
            interaction_elements,
        );
        component.evaluate(&mut context, &component_data, &mut accumulator);
        let claimed_sum =
            context.new_var(qm31_from_u32s(1398335417, 314974026, 1722107152, 821933968));
        accumulator.finalize_logup_in_pairs(
            &mut context,
            <TestComponentData as ComponentDataTrait<QM31>>::interaction_columns(&component_data),
            &component_data,
            claimed_sum,
        );

        let result = accumulator.finalize();
        let result_value = context.get(result);
        assert_eq!(result_value, BLAKE_COMPRESS_OPCODE_SAMPLE_EVAL_RESULT)
    }
}
