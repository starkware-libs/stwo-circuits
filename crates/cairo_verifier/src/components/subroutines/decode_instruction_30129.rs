...
pub fn accumulate_constraints<Value: IValue>(
    ...
    let [
        decode_instruction_30129_input_pc,
        offset0_col0,
        offset1_col1,
        offset2_col2,
        dst_base_fp_col3,
    ...
    let constraint_2_value = eval!(context, (op1_base_fp_col5) * ((1) - (op1_base_fp_col5)));
    acc.add_constraint(context, constraint_2_value);

    //Flag ap_update_add_1 is a bit.
    let constraint_3_value =
        eval!(context, (ap_update_add_1_col6) * ((1) - (ap_update_add_1_col6)));
    acc.add_constraint(context, constraint_3_value);

    // Use VerifyInstruction.
    let tuple_4 = &[
        eval!(context, 1719106205),
        eval!(context, decode_instruction_30129_input_pc),
        ...
        eval!(
            ...
            ((((dst_base_fp_col3) * (8)) + ((op0_base_fp_col4) * (16)))
                + ((op1_base_fp_col5) * (64)))
                + (((1) - (op1_base_fp_col5)) * (128))
        ),
        eval!(context, (ap_update_add_1_col6) * (32)),
    ...
    vec![
        ...
        eval!(context, (1) - (op1_base_fp_col5)),
    ]
}
