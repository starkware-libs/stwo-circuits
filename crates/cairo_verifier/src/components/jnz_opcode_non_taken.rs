...
pub fn accumulate_constraints<Value: IValue>(
    ...
    mem_verify::accumulate_constraints(
        &[
            eval!(
                context,
                (mem_dst_base_col7) + (decode_instruction_ad440_output_tmp_59be2_5_offset0)
            ),
            eval!(context, 0),
            eval!(context, 0),
            eval!(context, 0),
...
mod tests {
    ...
    fn test_evaluation_result() {
        ...
        assert_eq!(result_value, JNZ_OPCODE_NON_TAKEN_SAMPLE_EVAL_RESULT)
    }
}
