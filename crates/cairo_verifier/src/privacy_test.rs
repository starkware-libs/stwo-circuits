...
fn test_privacy_proof_info() {
    ...
    // Build the "standard circuit": the components ordered by ascending trace log size, exactly as
    // `CircuitStatement::new` and the prover order them. stwo commits the trace columns sorted by
    // size, so this is the committed-column order (see `sorted_component_order`). The proof size is
    // invariant to component order, but sorting here keeps this test measuring the same component
    // layout that the real prover and verifier use.
