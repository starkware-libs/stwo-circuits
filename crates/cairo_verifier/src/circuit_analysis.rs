    ...
    while let Some(var) = work.pop() {
        ...
        for &other_var in eq_adj.get(&var).into_iter().flatten() {
            ...
            if let Some(old_val) = insert_res {
                assert!(
                    old_val == val,
                    "Value mismatch for {other_var} during constant propagation"
                );
            } else {
                work.push(other_var);
            }
        ...
        for &gate_idx in consumers.get(&var).into_iter().flatten() {
            ...
            let (Some(&va), Some(&vb)) = (values.get(&arith.a), values.get(&arith.b)) else {
                continue;
            };

            let val = apply(arith.op, va, vb);
            let insert_res = values.insert(arith.out, val);
            if let Some(old_val) = insert_res {
                assert!(
                    old_val == val,
                    "Value mismatch for {} during constant propagation",
                    arith.out
                );
            } else {
                work.push(arith.out);
