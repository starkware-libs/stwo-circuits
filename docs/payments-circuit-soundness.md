...
| **Complementary live rows on one key defeat both absence claims.** Rows `(k, v, 0)` and `(k, 0, w)` are accepted — each fold sees one live entry at the position — and jointly act as the overwrite `v → w` while each row's absence claim is false. Same-side duplicates *are* rejected (duplicate position, one fold). Found by the Lean formalization: `update_sound` was unprovable as stated; the statement is now conditioned on live-key distinctness. | `complementary_rows_on_one_key_act_as_an_overwrite` (pins the accepted behavior), `same_side_duplicate_live_keys_are_rejected` | the batch producer: the dict squashing emits one row per key by construction; nonce batches are insert-only. Step 3 inherits this as an interface rule alongside the `(0, 0)` rule. |
